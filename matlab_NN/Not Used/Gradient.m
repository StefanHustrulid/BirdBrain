...
for constraint = 1:size(activations0(1,:)) %For each a0&y in batch calculate change in weights and balances
    ...
    dadz{1} = dActivationFunctiondz(activations0(:,constraint),weights{1},biases{1},activationFunctionType);
    if numel(activations)-1 > 1
        for layer = 2:numel(activations)-1
            dadz{layer} = dActivationFunctiondz(activations{layer-1},weights{layer},biases{layer},activationFunctionType);
        end
    end
    ...
    dCostdPreviousActivations{numberOfLayers-1} = dzdaprev{numberOfLayers-1}.*dcdz;
    ...
    for layer = numberOfLayers-2:-1:1
        ...
        dCostdPreviousActivations{layer-1} = dzdaprev{layer}.*(dadz{layer}.*(dCostdPreviousActivations{layer}));
